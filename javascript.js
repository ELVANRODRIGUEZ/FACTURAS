...
function findXMLElements(element) {
  ...
  rigthImpTag = xmlDoc2.getElementsByTagName("cfdi:Impuestos")[
    impTagPresent - 1
  ];

  if (rigthImpTag != undefined) {
    if (rigthImpTag.getAttribute("totalImpuestosTrasladados") != undefined) {
      totalImpTrasImpuestos = rigthImpTag.getAttribute(
    ...
    if (totalComprobante == 0 || !totalComprobante) {
      montoIVA0 = 0;
    } else if (totalComprobante - totalImpTrasImpuestos - montoBaseIVA < 0.75) {
      montoIVA0 = 0;
    } else {
    ...
    totalImpTrasImpuestos = parseFloat(0);
    totalIVA = parseFloat(0);
    montoAntesImp = parseFloat(totalComprobante);
    montoBaseIVA = parseFloat(0);
    montoSinIVA = parseFloat(totalComprobante);
    montoIVA0 = parseFloat(totalComprobante);
